...
<QuickAnswer tone="triage" label="First, isolate the cause">
- **Unplug everything on the circuit**, then try Reset. If it holds, an appliance on that chain is the culprit — plug items back in one at a time until it trips again.
- **If the GFCI won't reset even with nothing plugged in**, the problem is in the wiring or the device itself. That's an electrician call, not a DIY fix.
- **Stop and call immediately** if you see a **burnt smell, discolored outlet, warm faceplate,** or a **scorch mark** anywhere on the circuit.

A tripping GFCI is almost always doing its job. The goal isn't to silence it — it's to find what it's catching.
</QuickAnswer>
...
When it is time to swap the device itself, a modern [replacement GFCI outlet](https://www.amazon.com/s?k=15+amp+self+test+GFCI+outlet&tag=fixitreal-20) in the correct amperage (15A or 20A — match the breaker) with a weather-resistant rating for outdoor or damp locations is the right buy.
